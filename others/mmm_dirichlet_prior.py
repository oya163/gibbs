...
"""
    ...
    Usage of conjugate prior

    Best result: python mmm_dirichlet_prior.py -i 10 -e 1000 -k 3 -s 4
"""
...
def parse_args():
    ...
    parser.add_argument("-e", "--experiments", dest="experiments", type=int,
                        default=10, metavar="INT", help="Total Experiments [default:500]")
    parser.add_argument("-f", "--flips", dest="flips", type=int,
                        default=15, metavar="INT", help="Total flips per experiment [default:10]")
...
class MultinomialMixtureModel:
    def __init__(self, K, S, total_iteration, total_experimentation, total_flip_per_experimentation):
        ...
        self.beta = [np.random.random() for k in range(self.S)]
    ...
    def calculate_cluster_assignment_probability(self, index, X, init_cluster):
        ...
        # theta = np.random.dirichlet(np.array(theta), size=self.K)

        # Compute p_k (x_i | theta_k) for each cluster
        final_prob = []
        for k in range(self.K):
            final_prob.append((c_k[k] / (N + self.A - 1)) * self.g0(X[index], theta[k]))

        return theta, final_prob
    ...
    def display_actual(self):
        ...
        for k in range(self.K):
            print("Cluster Id: {}, Mixture: {:.3f}, Theta: {}".format(k, self.actual_mixture[k], self.init_theta[k]))

    # Display estimated parameters
    def display_estimated(self, init_cluster, theta):
        print("************ Estimated Parameters ************")
        cluster_type = Counter(init_cluster)

        for k in range(self.K):
            print(
                "Cluster Id: {}, Mixture: {:.3f}, Theta: {}".format(k, cluster_type[k] / len(init_cluster), theta[k]))
...
def main():
    ...
    # display estimated parameters
    model.display_estimated(init_cluster, theta)
